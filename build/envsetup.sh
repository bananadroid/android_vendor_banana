function __print_banana_functions_help() {
cat <<EOF
Additional BananaDroid functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- bananagerrit:    A Git wrapper that fetches/pushes patch from/to BananaDroid Gerrit Review.
- bananarebase:    Rebase a Gerrit change and push it again.
- bananaremote:    Add git remote for BananaDroid Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for BananaDroid Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the BananaDroid model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch banana_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/banana-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.banana.device | grep -q "$BANANA_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $BANANA_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function bananaremote()
{
    ...
    git remote rm banana 2> /dev/null
    ...
    then
        ...
        local PFX="bananadroid/"
    else
        local PROJECT=$REMOTE
    fi

    local BANANA_USER=$(git config --get review.review.bananadroid.org.username)
    if [ -z "$BANANA_USER" ]
    then
        git remote add banana ssh://review.bananadroid.org:29418/$PFX$PROJECT
    else
        git remote add banana ssh://$BANANA_USER@review.bananadroid.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'banana' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/bananadroid/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.banana.device | grep -q "$BANANA_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        bananaremote
        git push banana HEAD:refs/heads/'$1'
    fi
    '
}

function bananagerrit() {
    ...
    local user=`git config --get review.review.bananadroid.org.username`
...
function bananarebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "BananaDroid Gerrit Rebase Usage: "
        echo "      bananarebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.bananadroid.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.banana.device | grep -q "$BANANA_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $BANANA_FIXUP_COMMON_OUT ]; then
